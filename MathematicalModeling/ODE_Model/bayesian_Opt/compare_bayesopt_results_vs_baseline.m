%% compare_bayesopt_results_vs_baseline.m
% Loads fit_bayesopt_results.mat, runs the baseline model and the fitted
% BayesOpt model, and plots both model curves against the observed data.
%
...
outputFolder = 'bayesopt_vs_baseline_plots';
...
fprintf('Running BayesOpt fitted model...\n');
...
fig = figure('Units','normalized', ...
             ...
             'Name','BayesOpt fitted model vs baseline model');
...
title(tl, 'BayesOpt fitted model vs baseline model');
...
if SAVE_COMBINED_FIGURE
    ...
    combinedBase = fullfile(outputFolder, 'bayesopt_vs_baseline_combined');
...
if SAVE_INDIVIDUAL_PLOTS
    ...
    for plotIdx = 1:nPlots
        ...
        outBase = fullfile(outputFolder, sprintf('state_%02d_%s_bayesopt_vs_baseline', stateNum, safeName));
...
fprintf('\nDone. Compared BayesOpt fitted model against baseline model.\n');
...
function plotOneState(ax, plotIdx, stateNum, stateNames, stateYLabels, ...
    ...
    plot(ax, t_fit, sol_fit(:, stateNum), '-', ...
        'LineWidth', 1.8, ...
        'DisplayName', 'BayesOpt fitted model');
