...
function xdot = model(t,y,pars,intervention)

% Two independently controlled interventions are supported:
%
% intervention.pathogen.enabled
% intervention.pathogen.startTime
% intervention.pathogen.endTime
% intervention.pathogen.doseRate
%
% intervention.nitricOxide.enabled
% intervention.nitricOxide.startTime
% intervention.nitricOxide.endTime
% intervention.nitricOxide.inhibitionRate
%
% The antimicrobial uses a simple one-compartment PK/PD model:
%   dAbx/dt = doseRate(t) - kabx*Abx
%   uPathogen = kabxpe*Abx/(xabxpe + Abx)
% and adds -uPathogen*pe to the pathogen equation.
% The nitric-oxide inhibitor adds -uNO*no to the NO equation.



%----------------------------------Lags------------------------------------
        
limit_idx = [1 2 3 4 5 6 7 15 17 18];  % tnf il10 cxcl8 il6 ma mr pe no D Abx
...
% Antibiotic PK/PD parameters (115-117)
kabx   = pars(115);
kabxpe = pars(116);
xabxpe = pars(117);
...
% Antibiotic dosing is zero outside its treatment window. NO inhibition is
% zero outside its treatment window.
uPathogen        = 0;
uNO              = 0;
abxDoseRate      = 0;
...
if intervention.pathogen.enabled
    pathogenTreatmentIsActive = ...
        (t >= intervention.pathogen.startTime) && ...
        (t <= intervention.pathogen.endTime);

    uPathogen = kabxpe * abx / (xabxpe + abx);

    if pathogenTreatmentIsActive
        abxDoseRate = intervention.pathogen.doseRate;
    end
end
...
if intervention.nitricOxide.enabled
    noTreatmentIsActive = ...
        (t >= intervention.nitricOxide.startTime) && ...
        (t <= intervention.nitricOxide.endTime);

    if noTreatmentIsActive
        uNO = intervention.nitricOxide.inhibitionRate;
    end
end

%---------------------------Pathogen equation------------------------------
dpe    = kpg * pe * (1 - pe/peinf) ...
          - (kpm*sM*pe)/(muno+kmp*pe) ...
          - kpn*ma*pe*fs(il10,xI10,hmI10) ...
          - uPathogen*pe;

extinctionThreshold = 1e-8;

if pe <= extinctionThreshold && dpe < 0
    dpe = 0;
end

dD= kdn*f(alpha*no+pe,xdn,hmD) - mud*D;
dAbx = abxDoseRate - kabx*abx;
...
ft    =(kh*(HM - Hm)*((((abs(temp2 - Tm))^hht/((abs(temp2 - Tm))^hht + (abs(xht - Tm))^hht))))...
    *((((abs(xhp - BPo))^hhp/((abs((Vla/Cla) - BPo))^hhp + (abs(xhp - BPo))^hhp)))) + Hm);
...
dno   = knom*ma*f(tnf,xntnf,hntnf)*fs(il10,xn10,hn10) ...
      + knod*D*fs(il10,xn10D,hn10D) ...
      - kno*no ...
      - uNO*no;
