...
function [pars, Init] = load_pars_Init_Copeland_Edited(data)
...
kpm=1.8*.8; %1e-1 #91|.6|.31
muno=.02; %1e-3 #93|.002|.02
sM=.012; %.005
kmp=.008; %.01
% Super sensitive
kpn=.5*.008; %1e-2 #94|1e-2 | .5*.008
% Saturation Constants 
xI10=20; 
% Exponent %99
hmI10=2; % 3-6 similiar graphs

% Antibiotic PK/PD parameters
kabx=0.5;      % first-order antibiotic clearance, 1/time
kabxpe=5.0*2;    % maximum added Pe clearance rate, 1/time
xabxpe=1.0;    % antibiotic concentration producing half-maximal kill


% Damage Model dD
kdn= 2; %.35  %95 | 2
mud= 2 ; %96 % 0.017|0.17|.37 
xdn= 10; %97 .06 | 10
hmD= 6; %107 2 
alpha= 100; %98 3|100
...
% NO Added PARS
knod=.002; %108 2| 0.002
xnDl10=10; %109, 30|7  Up Damage
hnDl10=1; %110  2|     Up Damge
xn10D=10; %111 30|      Down Il10
hn10D=2; %112        Down Il10



%% MA pars Added for Damage
kD=20*1.5;
xDam=30;
hmDa=2;
xm10D=30;
...
% HM   = 207 - 0.7 * age; % Value 186.7
HM=hr_max;
kh   =.55*2;  % .55|
tau2 = 2;      % 2|

xht  = 36.4;     % 36.4 par-62
hht  = 2;        % 2|
...
%Exponents
hm10   = 1;	    % Not estimated 
hmtnf  = 3;	    % Not estimated 
hmpe   = 1;         % 1| 
...
%-------------------Antibiotic PK state------------------------------------
% One-compartment antibiotic concentration used by the intervention model.
abxI = 0;
Init = [Init abxI];
